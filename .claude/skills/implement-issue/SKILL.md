...
- **`Closes #N` no corpo do PR não fecha a issue automaticamente**: como os PRs de feature miram `develop` (não a branch default `main`), o fechamento automático do GitHub não dispara ao mergear. Quando alguém mergear o PR na `develop`, é preciso fechar a issue manualmente (`gh issue close <numero> --comment "..."`) e mover o card para "Done" — isso não é responsabilidade deste comando (que não faz merge), mas de quem mergeia, ou de uma futura automação equivalente ao `branch-source-guard.yml`. Ao rodar `/implement-issue` sem argumento, o passo de resolução do próximo item do backlog corrige inconsistências desse tipo que encontrar pelo caminho (ver acima).
